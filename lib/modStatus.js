...
function modFileDateCheck() {
  _fs_.stat(_globals_.globals.modListFile, (error, stats) => {
	if (error) {
		_fs_.appendFile(_globals_.globals.modListFile, '', (error) => {
			if (error) throw error;
			console.log('Created file: ' + _globals_.globals.modListFile);
		});
	} else {
  ...
  _fs_.stat(_globals_.globals.modListUsernames, (error, stats) => {
	if (error) {
		_fs_.appendFile(_globals_.globals.modListUsernames, '', (error) => {
			if (error) throw error;
			console.log('Created file: ' + _globals_.globals.modListUsernames);
		});
	} else {
		const currentTimeMS = Date.now();
		const fileModDate = Date.parse(stats.mtime);
